...
task BamMapMarkSort {
  ...
  command <<<
      set -euo pipefail
      ${tool_path}/minimap2/minimap2 ${minimap2_options} ${ref_mmi} ${input1_fq} ${input2_fq} \
      | \
      ${tool_path}/samblaster \
      | \
      ${tool_path}/samtools sort -m ${sort_memory} -@ ${cores} -l 0 - \
      | \
      tee ${output_bam_basename}.bam \
      | \
      ${tool_path}/samtools index - ${output_bam_basename}.bai
